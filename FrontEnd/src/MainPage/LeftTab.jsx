...
export function LeftTab({current}) {
    ...
    return(
        <div onClick={() => setLogoutPop(false)} className="flex flex-col w-72 border border-r-gray-300 items-center relative ">
            ...
            <button onClick={() => {if(user.user.step != 2){navigate('/setup/food')}else{navigate('/profile')}}} className={`w-[95%] h-12 flex flex-row justify-center items-center rounded-full m-1 mt-4  border-2 border-blue-500 hover:drop-shadow-lg ${selected == 'Profile' ? 'bg-blue-500 hover:bg-blue-600' :  'bg-white  hover:bg-gray-200'}`}>
